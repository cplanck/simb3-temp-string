...
void readTopString() {
  ...
  if (digitalRead(2) == HIGH) {
    ...
    onBoardFlash.writebytes("TOPSTRING_ROMS.csv", ROMarray);
  }
  
  // if pin 2 is NOT pulled high, retrieve ROMs from onboard Flash memory
  else {
    Serial.println("Getting temperature bytes...");
    onBoardFlash.readbytes("TOPSTRING_ROMS.csv", ROMarray);
    for (int i = 0; i < chipNum; i++) {
      ds.reset();
      chipSelect(i);
    }
